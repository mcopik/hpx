    ...
    Vector& a, Vector& b, Vector& c)
{
    ...
    Concurrency::accelerator ampAccelerator;
    ...
    std::wcout << ampAccelerator.get_description() << std::endl;
    for(std::size_t iteration = 0; iteration != iterations; ++iteration)
    {
	    ...
	    hpx::parallel::copy(hpx::parallel::gpu.with(hpx::parallel::static_chunk_size(1)),
                accView, part_size, buffera, bufferc);
        accView.wait();
        buffera.buffer->get_accelerator_view().wait();
        bufferc.buffer->get_accelerator_view().wait();
  ...
  //  buffera.queue.wait();
   ...
   // bufferc.queue.wait();
        ...
        hpx::parallel::transform(hpx::parallel::gpu.with(hpx::parallel::static_chunk_size(1)),
            accView, part_size, bufferc, bufferb,
            [=](STREAM_TYPE v1) {
                    //printf("%lu \n", v);
                    //return v1 + k*v2;
                    return scalar * v1;
  ...
  //  bufferc.queue.wait();
        ...
        t1 = std::chrono::high_resolution_clock::now();
	    hpx::parallel::transform(hpx::parallel::gpu.with(hpx::parallel::static_chunk_size(1)),
            accView, part_size, buffera, bufferb, bufferc,
